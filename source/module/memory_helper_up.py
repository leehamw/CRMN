...
# -*- coding: utf-8 -*-
"""
File: source/module/memory_helper_up.py
"""
...
class UserProfileMemory(nn.Module):
    def __init__(self,
                 ...
                 mode="mlp",
                 use_gpu=False):
        super(UserProfileMemory, self).__init__()
    ...
    def load_memory(self, kb_inputs, enc_hidden, mask=None):
        kb_memory_list = []
        q = [enc_hidden.squeeze(0)]
        batch_size, kb_num = kb_inputs.size(0), kb_inputs.size(1)
        kb_inputs_mask = kb_inputs.eq(0).eq(0).float()

        for hop in range(self.max_hop):
            embed_state = self.C[hop](kb_inputs)
            embed_state = embed_state * (kb_inputs_mask.unsqueeze(-1).expand_as(embed_state))
            ...
            embed_state_next = embed_state_next * (kb_inputs_mask.unsqueeze(-1).expand_as(embed_state_next))
