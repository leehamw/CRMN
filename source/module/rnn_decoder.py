...
from source.module.memory_helper_up import UserProfileMemory
...
class RNNDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.user_profile_memory = UserProfileMemory(vocab_size=self.kb_output_size,
                                                     query_size=self.hidden_size,
                                                     memory_size=self.memory_size,
                                                     max_hop=self.max_hop,
                                                     padding_idx=self.padding_idx,
    ...
    def initialize_kb_v3(self, kb_inputs, enc_hidden, kb_mask):
        kb_memory, selector, kb_readout = self.kb_memory_v3.load_memory(kb_inputs,
                                                                        enc_hidden,
                                                                        kb_mask)
        return kb_memory, selector, kb_readout

    def initialize_user_profile(self, user_profile_inputs, enc_hidden, user_profile_mask):
        up_memory, up_readout = self.user_profile_memory.load_memory(user_profile_inputs,
                                                                     enc_hidden,
                                                                     user_profile_mask)
        return up_memory, up_readout
