...
class Field(object):
    ...
    Field
    """
    def __init__(self, sequential=False, dtype=None):
...
class TextField(Field):
    ...
    def __init__(self,
                 ...
                 special_tokens=None,
                 embed_file=None):
        super(TextField, self).__init__(sequential=True, dtype=int)
    ...
    def build_word_embeddings(self, embed_file):
        ...
        if isinstance(embed_file, list):
            ...
            for word, index in self.stoi.items():
                if '*' in word:
                    words = word.split('*')
                    embeddings = list(map(lambda e: embeds[self.stoi.get(e, self.stoi[UNK])], words))
                    embeds[index] = list(np.mean(embeddings, axis=0))
                    cover += 1
