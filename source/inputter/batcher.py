...
class DialogBatcher(object):
    ...
    def create_batches(self, data):
        ...
        situation = [sample['situation'] for sample in sorted_data]
        user_profile = [sample['user_profile'] for sample in sorted_data]
        max_turn = max(turns)
        inputs = []
...
def create_user_profile_batch(user_profile_list):
    ...
    """
    user_profile_list = list(map(lambda xs: [x[1:-1] for x in xs], user_profile_list))
