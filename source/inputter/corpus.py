...
class KnowledgeCorpus(object):
    ...
    def build_vocab(self, data):
        ...
        for name in data[0].keys():
            if name in ['situation', 'user_profile']:
                field = self.fields.get('kb')
            else:
            ...
            if isinstance(field, TextField):
                ...
                if name == 'kb':
                    xs += [list(map(lambda s: s.replace('*', ' '), x[name])) for x in data]
    ...
    def build_examples(self, data):
        ...
        for raw_data in tqdm(data):
            ...
            for name, strings in raw_data.items():
                ...
                elif name in ['situation', 'user_profile']:
                    example[name] = self.fields['kb'].numericalize(strings)
                else:
