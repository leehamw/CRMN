...
from source.utils.rewards import reward_fn1
...
class Seq2Seq(BaseModel):
    ...
    def __init__(self,
        ...
        self.situation_encoder = RNNEncoder(input_size=self.embed_size,
                                            hidden_size=self.hidden_size,
                                            embedder=self.enc_embedder,
                                            num_layers=self.num_layers,
                                            bidirectional=self.bidirectional,
        ...
        self.situation_bridge = nn.Sequential(
            nn.Linear(self.hidden_size * 2, self.hidden_size),
            nn.ReLU(),
        )

        self.user_profile_bridge = nn.Sequential(
            nn.Linear(self.hidden_size * 2, self.hidden_size),
            nn.ReLU(),
        )

        self.kb_readout_bridge = nn.Sequential(
            nn.Linear(self.hidden_size * 2, self.hidden_size),
            nn.ReLU(),
        )
    ...
    def load_user_profile_memory(self, kb_inputs, user_profile_inputs):
        """
        load user profile memory
        """
        user_profile, user_profile_lengths = user_profile_inputs
        if self.use_gpu:
            user_profile = user_profile.cuda()
            user_profile_lengths = user_profile_lengths.cuda()

        self.user_profile = user_profile

        user_profile_mask = user_profile_lengths.eq(0)  # (batch_size, profile_num)
        # user_profile_mask = user_profile.eq(0).eq(0)  # (batch_size, profile_num, profile_term)
        self.user_profile_mask = user_profile_mask

    def load_situation_memory(self, situation_inputs):
        """
        load situation memory
        """
        if self.use_gpu:
            situation_inputs = tuple([ele.cuda() for ele in situation_inputs])
        enc_outputs, enc_hidden = self.situation_encoder(situation_inputs)

        if self.with_bridge:  # todo 1. change to bidirectional 2. change a bridge
            enc_hidden = self.bridge(enc_hidden)

        self.situation = enc_hidden
    ...
    def encode(self, enc_inputs, hidden=None):
        ...
        enc_hidden = self.situation_bridge(torch.cat([enc_hidden, batch_situation], dim=-1))

        up_memory, up_readout = self.decoder.initialize_user_profile(batch_user_profile,
                                                                     enc_hidden,
                                                                     batch_user_profile_mask)
        enc_hidden = self.user_profile_bridge(torch.cat([enc_hidden, up_readout.unsqueeze(0)],
                                                        dim=-1))

        kb_memory, selector, kb_readout = self.decoder.initialize_kb_v3(batch_kb_inputs,
                                                                        enc_hidden,
                                                                        kb_mask)
        enc_hidden = self.kb_readout_bridge(torch.cat([enc_hidden, kb_readout.unsqueeze(0)],
                                                      dim=-1))
    ...
    def sample(self, enc_inputs, dec_inputs, hidden=None, random_sample=False):
        ...
        pred_log_logits = torch.zeros((batch_size, max_len, self.tgt_vocab_size))
        pred_word = torch.ones((batch_size, max_len),
                               dtype=torch.long) * self.padding_idx  # here, zeros is equal to the padding idx
    ...
    def collect_rl_metrics(self, sample_outputs, greedy_outputs, target, gold_entity,
                           ptr_index, kb_index, target_mask, task_label):
        """
        collect_rl_metrics
        """
        num_samples = target.size(0)
        ...
        # log prob for sampling and greedily gen
        ...
        # cal reward
        sample_reward, _, _ = self.reward_fn(sample, target, gold_entity, ptr_index, task_label)
        greedy_reward, bleu_score, f1_score = self.reward_fn(greedy_sample, target, gold_entity, ptr_index, task_label)
        ...
        nll = sample_log_prob * reward.to(
            sample_log_prob.device)  # reward: [batch_size, 1] or the broadcast of [batch_size, 1]
    ...
    def iterate(self, turn_inputs, kb_inputs, situation_inputs=None, user_profile_inputs=None,
