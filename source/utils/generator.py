...
from source.inputter.batcher import create_turn_batch, create_kb_batch, create_situation_batch, \
...
class BeamGenerator(object):
    ...
    def forward(self, turn_inputs, kb_inputs, situation_inputs=None, user_profile_inputs=None, enc_hidden=None):
        """
        forward
        """
        ...
        with torch.no_grad():
            ...
            self.model.load_situation_memory(situation_inputs)
            self.model.load_user_profile_memory(kb_inputs, user_profile_inputs)

            last_turn_num = len(turn_inputs)
            ...
            for i, inputs in enumerate(turn_inputs):
                ...
                if not (self.mode == 'test' and i != (last_turn_num - 1)):
                ...
                if not (self.mode == 'test' and i != (last_turn_num - 1)):
                    ...
                    if self.mode == 'test':
                        assert tgt_text == ['<TEST_RESPONSE>']
                        assert i == (last_turn_num - 1)
                    ...
                    enc_outputs.add(src=src_text, tgt=tgt_text, preds=preds_text, task=task,
                                    gold_entity=gold_entity, scores=scores)
    ...
    def generate(self, data_iter, save_file=None, verbos=False):
        ...
        for batch_idx in tqdm(range(num_batches)):
            ...
            result_batch = self.forward(turn_inputs=turn_inputs, kb_inputs=kb_inputs,
                                        ...
                                        user_profile_inputs=user_profile_inputs, enc_hidden=None)
        ...
        for src, hyp, ref, tk, ent in zip(srcs, hyps, refs, tasks, gold_entity):
            src_str = " ".join(src)
            hyp_str = " ".join(hyp)
            ref_str = " ".join(ref)
            if self.mode == 'test':
                sample = {"source": src_str,
                          "result": hyp_str,
                          "target": ref_str}
            else:
                sample = {"result": hyp_str,
                          "target": ref_str,
                          "task": tk,
                          "gold_entity": ent}
