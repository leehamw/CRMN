...
def gen_others_2(dialog, kb):
    ...
    for s in range(1, len(dialog), 2):
        ...
        for triple in kb:
            ...
            if new_obj in sys:
                kb_ptr.append(1)
                dialog[s] = sys.replace(new_obj, old_obj)
                gold.append(old_obj)
            else:
                kb_ptr.append(0)
...
def convert_text_for_model(input_file, out_file, mode):
    ...
    with open(input_file, 'r', encoding='utf-8') as fr:
        for line in fr:
            ...
            kb = gen_kb(knowledge)
            kb_index, gold_entity = gen_others_2(dialog, kb)  # todo change to gen_others
            sample = {
                'task': gen_task(),
