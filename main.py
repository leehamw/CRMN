...
def model_config():
    ...
    data_arg.add_argument("--embed_file", type=str, default=None)  # todo None

    # Network
    net_arg = parser.add_argument_group("Network")
    net_arg.add_argument("--embed_size", type=int, default=200)   # todo 300 for pre_train embeddings
    net_arg.add_argument("--hidden_size", type=int, default=256)
    net_arg.add_argument("--bidirectional", type=str2bool, default=False)
    net_arg.add_argument("--max_vocab_size", type=int, default=30000)  # todo vocab 30000
    ...
    train_arg.add_argument("--use_embed", type=str2bool, default=False)  # todo init True
    train_arg.add_argument("--train_embed", type=str2bool, default=True)  # todo init False
    train_arg.add_argument("--log_steps", type=int, default=5)
    train_arg.add_argument("--valid_steps", type=int, default=100)  # init 20

    # Generation
    gen_arg = parser.add_argument_group("Generation")
    gen_arg.add_argument("--test", action="store_true")
    gen_arg.add_argument("--mode", type=str, default="valid", choices=['valid', 'test'])
...
def main():
    ...
    if config.test and config.ckpt:
        data_iter = corpus.create_batches(batch_size=1 if config.mode == 'test' else config.batch_size,
        ...
        print("Generating ...")
        generator.generate(data_iter=data_iter, save_file=config.save_file, verbos=True)
